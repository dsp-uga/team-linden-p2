...
my $highLightVideoOut = "highlightVideo.gif";
my $sourceMaskFile = "";
my $predictMaskFile = "";
my $newSourceMask = "";
my $newPredictMask = "";
...
sub ReadCmdLine {
    for( my $i = 0; $i < @ARGV; $i++ ){
        if( $ARGV[$i] eq "-d" ){
            ...
            $i++;
            $highLightVideoOut = $ARGV[$i];
        } elsif( $ARGV[$i] eq "-s" ){
            $i++;
            $sourceMaskFile = $ARGV[$i];
        } elsif( $ARGV[$i] eq "-p" ){
            $i++;
            $predictMaskFile = $ARGV[$i];
        }
    }

    if( $sourceDir eq "" or $sourceMaskFile eq "" or $predictMaskFile eq "" ){
        die <<EOF;
ERROR: Missing command line arguments.
./genHightlightVideo.pl -d <directory containing all frame png files>
    -s <correct mask png file>
    -p <predicted mask png file> 
    (optional: 
        -f <output gif film filename [def. fullVideo.gif]>
        -h <output gif highlighted film filename [def. highlightVideo.gif]>
    )
>>> WARNING <<<
...
EOF
    ...
    if( $fullVideoOut !~ /\.gif$/ or $highLightVideoOut !~ /\.gif$/ ){
        die "ERROR: Please specify output film filenames ending with '.gif'\n";
    }
    if( $sourceMaskFile !~ /\.png/ or $predictMaskFile !~ /\.png/ ){
